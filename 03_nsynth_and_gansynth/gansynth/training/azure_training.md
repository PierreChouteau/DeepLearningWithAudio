...
```
./dlwa.py gansynth train --dataset_name your_name/mydataset --model_name your_name/model
```

This command line will start the GANSynth training. It will load the training data that is stored in the 2 files, `data.tfrecord` and `meta.json`, and generate checkpoints in the `gansynth/your_name/mysound` directory

Note:
* **your_name/mydataset** and  **your_name/model** should be replaced with your own folder names.
